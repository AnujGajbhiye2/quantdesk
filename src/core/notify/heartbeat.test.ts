...
describe('buildHeartbeatText', () => {
  it('includes seq number in header', () => {
    const text = buildHeartbeatText(
      { totalBars: 200, symbolCount: 50, refreshErrors: 0, post: { pendingFills: { results: [] }, sweep: { results: [] }, scan: { result: null }, edge: { result: null } } },
      baseOpts,
    );
  ...
  it('shows bars ingested and symbol count', () => {
    const text = buildHeartbeatText(
      { totalBars: 347, symbolCount: 503, refreshErrors: 0, post: { pendingFills: { results: [] }, sweep: { results: [] }, scan: { result: null }, edge: { result: null } } },
      baseOpts,
    );
  ...
  it('shows ERROR tag when refresh had errors', () => {
    const text = buildHeartbeatText(
      { totalBars: 100, symbolCount: 10, refreshErrors: 3, post: { pendingFills: { results: [] }, sweep: { results: [] }, scan: { result: null }, edge: { result: null } } },
      baseOpts,
    );
  ...
  it('renders INACTIVE drawdown breaker branch', () => {
    const text = buildHeartbeatText(
      { totalBars: 0, symbolCount: 0, refreshErrors: 0, post: { pendingFills: { results: [] }, sweep: { results: [] }, scan: { result: null }, edge: { result: null } } },
